...
// Creates an octree with a max level and ideal entity count
MyOctant::MyOctant(uint a_nMaxLevel, uint a_nIdealEntityCount)
{
	// Call Init to initialize the variables to the correct default values
	Init();
...
{
	// Swap the variables of the octants
	...
	// Swap the children of the octants
	for (uint i = 0; i < 8; i++) 
	{
...
{
	if (m_uChildren != 0) { return; } // If there are already children, do not subdivide again
...
{
	if (m_uChildren == 0) // Check if there are branches to break
	{
		return;
	}
